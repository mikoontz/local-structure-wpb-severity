...
source(here::here("workflow/18a_tree-detection-helper-functions.R"))
...
suppressWarnings(suppressMessages( # Suppress all the outputs from lidR functions. Too much red text!
  ttops_summary <- 
    ...
    furrr::future_map(.f = function(current_plot) {
      ...
      lmfx_params <-
        tidyr::crossing(dist_2d_vals, ws_vals) %>%
      ...
      ttops_sf <-
        lapply(seq_along(ttops), FUN = function(j) {
          ...
          NO_TREES_DETECTED <- nrow(ttops[[j]][[ttops_sf_idx]]) == 0
          ...
          if(!NO_TREES_DETECTED) {
            nn <- nngeo::st_nn(x = ttops[[j]][[ttops_sf_idx]], 
                               y = ttops[[j]][[ttops_sf_idx]], 
                               k = min(c(4, nrow(ttops[[j]][[ttops_sf_idx]]))), 
                               returnDist = TRUE, sparse = FALSE, progress = FALSE)$dist %>% 
              do.call("rbind", .) %>% 
              as_tibble() %>% 
              dplyr::select(-1)  # remove nuisance column (distance between each tree and itself, which is always 0)
          } else {
            nn <- tibble()
          }
          ...
          if (ncol(nn) == 0) {nn <- tibble(nn1 = NA_real_, nn2 = NA_real_, nn3 = NA_real_)} else
            if (ncol(nn) == 1) {nn <- nn %>% dplyr::rename(nn1 = V2) %>% dplyr::mutate(nn2 = NA_real_, nn3 = NA_real_)} else
              if (ncol(nn) == 2) {nn <- nn %>% dplyr::rename(nn1 = V2, nn2 = V3) %>% dplyr::mutate(nn3 = NA_real_)} else
      ...
      ttops_time <-
        lapply(seq_along(ttops), FUN = function(k) {
          ...
          dplyr::tibble(ttops_method = names(ttops[k]), elapsed_time = ttops_time)
        }) %>% do.call("rbind", .)
...
total_time <- as.numeric(difftime(time1 = Sys.time(), time2 = start, units = "secs"))
