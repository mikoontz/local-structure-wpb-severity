...
best_algorithm_deets_print <-
  ...
  dplyr::rename(`Forest metric` = forest_metric,
                `Correlation with ground` = ground_correlation,
                RMSE = rmse,
                # `Mean error` = me,
                # `Median error` = med_error,
                `Correlation within top 5%?` = ground_cor_within_threshold_pct,
                `RMSE within top 5%?` = rmse_within_threshold_pct)

knitr::kable(best_algorithm_deets_print, digits = 2, format = "pandoc")
```
