...
```{r site_characteristics}
# Need a table that includes the site name, elevation, CWD, surveyed area
```
...
```{r pp_check_visuals}
# Plot of the posterior predictive check and the zoomed in version at 0?
```

```{r summary_of_model}
# Would be good to have a summary table of the model
...
Use drones for insect attack studies! [@morris2017].
...
We used 4 chains with 2000 iterations each (1000 warmup, 1000 samples), and confirmed chain convergence by ensuring all `Rhat` values were less than 1.1 [@brooks1998]. We used posterior predictive checks to visually confirm model performance by overlaying the density curves of the predicted number of dead trees per cell over the observed number [@gabry2019]. We used 50 random samples from the model fit to generate 50 density curves and ensured curves were centered on the observed distribution, paying special attention to model performance at capturing counts of zero.
...
![Example digital surface model (DSM) that is a direct output from the dense point cloud generated using structure from motion (SfM) processing. The DSM represents the ground elevation plus the vegetation height.](../../figures/eldo_3k_3_dsm.png)

![Point cloud from Pix4D](../../figures/eldo_3k_3_point_cloud.png)

![Example digital terrain model (DTM) resulting from processing the digital surface model using the cloth simulation filter algorithm [@zhang2016], which classifies points in the dense point cloud as "ground" or "not-ground" and then interpolates the "ground" elevation for the rest of the dense point cloud footprint. The DTM represents the ground elevation without any vegetation.](../../figures/eldo_3k_3_dtm.png)

![Example canopy height model (CHM) generated by subtracting the digital terrain model from the digital surface model. The CHM represents the height of all of the elevation above ground level.](../../figures/eldo_3k_3_chm.png)

![Tree top detection](../../figures/eldo_3k_3_ttops.png)

![Crown segmentation](../../figures/eldo_3k_3_crowns.png)

![Live/dead classification](../../figures/eldo_3k_3_live_dead.png)

![Host/non-host classification](../../figures/eldo_3k_3_host_nonhost.png)

![Halfeye plot showing posterior distributions of the effect sizes for the model](../../figures/effect-sizes-halfeye.png)

![Raster version of model results showing the primary influence of Ponderosa pine structure on the probability of ponderosa pine mortality, and the interaction across climatic water deficit](../../figures/pipo_tpha_qmd_cwd_interaction_raster.png)

![Line version with credible intervals showing primary influence of Ponderosa pine structure on the probability of ponderosa pine mortality, and the interaction across climatic water deficit](../../figures/pipo_tpha_qmd_cwd_interaction.png)
...
The western pine beetle is known to exhibit strong aggregation and anti-aggregation behavior arising from its pheromone communication, and thus it is likely that the measured spatial covariance in this study is attributable to the magnitude of this effect at each site. 
