...
site_characteristics <- 
  ...
  tidyr::separate(site, into = c("Forest", "Elevation band", "Rep"), remove = TRUE) %>% 
  dplyr::mutate(Mortality = paste(sprintf("%0.2f", prop_mortality), sprintf("%0.2f", ground_mortality), sep = "/"),
                `Density (tpha)` = paste(sprintf("%0.2f", air_tpha), sprintf("%0.2f", ground_tpha), sep = "/")) %>% 
  ...
  dplyr::select(Forest, `Elevation band`, Rep, `CWD (mm)`, `CWD (z-score)`, `Survey area (ha)`, Mortality, `Density (tpha)`)
...
kable(site_characteristics, digits = 2, format = "pandoc", caption = "Site characteristics for each of the 32 sites.")
```
