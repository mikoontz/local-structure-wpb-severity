...
for (i in seq_along(sites_to_process)) {
  ...
  if (sites_checklist$overwrite[sites_checklist$site == current_site]) {
    ...
    for (j in seq_along(plots_visible_from_ortho)) {
      ...
      current_plot_dtm <- raster::crop(site_dtm, current_plot_boundary)
      current_plot_dsm <- raster::crop(site_dsm, current_plot_boundary)
      ...
      # dsm
      writeRaster(x = current_plot_dsm, filename = paste0(current_dir, current_site, "_plot-remote-data/", current_site, "_", current_plot_id, "_dsm.tif"))
      # dtm
      writeRaster(x = current_plot_dtm, filename = paste0(current_dir, current_site, "_plot-remote-data/", current_site, "_", current_plot_id, "_dtm.tif"))
      # las
